...
smith_w_main = np.zeros((len(sec1) + 1, len(sec2) + 1))
sw_zeros_check = np.zeros((len(sec1), len(sec2)))
print(sw_zeros_check)
print(sec1)
print(sec2)
...
for i in range(len(sec1)):
    for j in range(len(sec2)):
        if sec1[i] == sec2[j]:
            sw_zeros_check[i][j] = right
        else:
            sw_zeros_check[i][j] = wrong

for i in range(len(sec1) + 1):
    smith_w_main[i][1] = i * gap
for j in range(len(sec2) + 1):
    smith_w_main[1][j] = j * gap
...
def backtrack(secventa1, secventa2, main_array, zeros_check):
    ...
    while ti > 0 and tj > 0:
        ...
        if (ti > 0 and tj > 0 and main_array[ti][tj] == main_array[ti - 1][tj - 1] + zeros_check[ti - 1][
            ...
            aligned_1 = secventa1[ti - 1] + aligned_1
            aligned_2 = secventa2[tj - 1] + aligned_2
...
initializare(sec1, sec2, smith_w_main, sw_zeros_check)
backtrack(sec1, sec2, smith_w_main, sw_zeros_check)
